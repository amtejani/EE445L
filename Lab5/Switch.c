...
void Switch_Init(void(*button1Task)(void), void(*button2Task)(void),
  ...
  GPIO_PORTF_IBE_R |= 0x13;     //     PF4 is both edges
  GPIOArm();
  ...
  Touch3 = 0;   
  Release3 = 0;
  Last = (PF4 | PF1 | PF0);     // initial switch state
 }
// Interrupt on rising or falling edge of PF4 (CCP0)
void GPIOPortF_Handler(void){
  GPIO_PORTF_IM_R &= ~0x13;     // disarm interrupt on PF4 
  ...
  if(Last == 0x03){    			// button 3 press, PF4
    (*Button3Task)(); 			// execute user task
  } else if(Last == 0x11){   // button 2 press, PF1
    (*Button2Task)();  			// execute user task
  } else if(Last == 0x12){   // button 1 press, PF0
